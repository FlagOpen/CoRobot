...
**DataCollect**是一套基于Lerobo框架构建的多本体数据采集系统。系统由[RoboDriver](https://github.com/FlagOpen/RoboDriver)、[RoboDriver-Server](https://github.com/FlagOpen/RoboDriver-Server)和[RoboXstudio](https://ei2data.baai.ac.cn/home)三部分组成：**RoboDriver**为各类机器人本体驱动软件的开发工具，提供多协议数据流的接入框架；**RoboXstudio**作为一站式具身数据平台，包含数据采集任务下发、数据管理和数据标注等功能；**RoboDriver-Server**作为连接RoboDriver与RoboXstudio的通道，实现数据的转发与同步。
...
## DataConvert
**DataConvert**是一个专门用于具身数据格式转换的工具集，支持主流具身数据格式RLDS、HDF5、LeRobotDataset、JSONL与LeRobot格式的双向转换，满足模型训练的多元化数据格式需求。
